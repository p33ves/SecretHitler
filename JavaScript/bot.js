...
function sendRoles(playerList, channelID) {
  ...
  fullDetailsOfPlayer = Object.keys(playerList).map((playerId, i) => ({
    playerId,
    name: playerList[playerId],
    role: roles[i],
  }));
  ...
  if (message.substring(0, 3) == "sh!") {
    ...
    switch (cmd) {
      ...
      case "test":
        bot.uploadFile({
          to: channelID,
          file: "./images/WelcomeToSecretHitler.png",
      ...
      case "begin":
        if (
          ...
          Object.keys(playerList).length < 11
        ) {
          bot.sendMessage({
            to: channelID,
            message: dedent(
          ...
          if (Object.keys(playerList).length < 5) {
            ...
            bot.sendMessage({
              to: channelID,
              message:
                "Sorry, too many players. Maximum player limit per game is 10!",
            });
          } else {
            bot.sendMessage({
              to: channelID,
